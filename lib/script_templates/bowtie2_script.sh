/usr/local/bin/bowtie2 -p 6 BOWTIE2_OPTIONS -x BOWTIE2_INDEX  --un-conc-gz OUTPUT_DIR/OUTFILE -1 READ1 -2 READ2 > /dev/null 2> OUTPUT_DIR/ERR_OUTFILE && (echo "SAMPLE_NAME" >> OUTPUT_DIR/samples_ok.txt) || (echo "$HOSTNAME SAMPLE_NAME" >> OUTPUT_DIR/samples_ko.txt)
